...
public class FlowConversionTest {
	...
	public void minimal() {
		...
		Utils.assertLangString(flow.getName(), "Flow");
		...
		Utils.assertNull(classification.classifications);
		Utils.assertNull(classification.compartmentLists.get(0).compartments);
		Utils.assertNull(info.casNumber, info.sumFormula, info.synonyms, flow.flowInfo.geography);
	...
	public void complete() {
		...
		DataSetInfo info = flow.flowInfo.dataSetInfo;
		Utils.assertLangString(info.generalComment, "This is a complete elementary flow");
		Assert.assertEquals("12345-67-8", info.casNumber);
		Assert.assertEquals("O2", info.sumFormula);
		Utils.assertLangString(info.synonyms, "Synonym");
		Utils.assertLangString(flow.flowInfo.geography.location, "DE");
		List<FlowPropertyRef> properties = flow.flowPropertyList.flowProperties;
	...
	private void assertFlowProperty(List<FlowPropertyRef> properties, int index, int id, String uuid, String name,
		...
		Utils.assertLangString(property.flowProperty.name, name);
	...
	private Flow convert(String testType, String id) {
		...
		FlowConverter converter = new FlowConverter(config);
		JsonObject flow = config.store.get("Flow", id);
